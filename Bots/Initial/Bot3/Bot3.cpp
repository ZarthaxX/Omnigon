	...
	while (number_of_games > 0) {
		...
		while (true) {
			...
			for (Action action : actions) {
				...
				if (temp_enemy_amount < best_enemy_amount) {
					best_action = action;
					best_threat = number_of_threats(temp_board, my_id);
					best_enemy_amount = temp_enemy_amount;
				}
				else if(temp_enemy_amount == best_enemy_amount){
					int temp_threat = number_of_threats(temp_board, my_id);
